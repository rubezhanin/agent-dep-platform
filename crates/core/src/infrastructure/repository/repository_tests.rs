...
async fn record_snapshot_persists_divisions_agents_and_files() {
    ...
    let (result, report) = IngestService::new().ingest_local(&src).expect("ingest");
...
async fn record_snapshot_supersedes_previous_active() {
    ...
    // First ingest.
    let (r1, rep1) = IngestService::new().ingest_local(&src).expect("ingest 1");
    ...
    let (r2, rep2) = IngestService::new().ingest_local(&src).expect("ingest 2");
...
async fn record_snapshot_does_not_supersede_when_status_not_active() {
    ...
    let (mut r, rep) = IngestService::new().ingest_local(&src).expect("ingest");
